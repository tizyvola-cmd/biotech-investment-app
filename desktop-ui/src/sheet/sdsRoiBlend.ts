/**
 * Multi-curve ROI blend (client) — mirrors prediction/sds_roi_blend.py.
 */
...
import { SUPERNova_OFFSETS, SUPERNova_HISTORY_CURVE } from "./sdsHistoryCurve";
...
export type SdsCurveRoiHorizon = {
  /** % vs T−60 on the blended μ curve at the calendar knot. */
  pct_vs_m60: number;
  /** Optional delta vs the blended level at ``now_offset``. */
...
export type SdsCurveRoiBlend = {
  ...
  /** Curve μ blend (secondary reference). */
  horizons_curve?: Partial<Record<SdsCurveRoiHorizonKey, SdsCurveRoiHorizon>>;
  /** Raw Prediction+Recalibration at standard knots (baseline for blend eval). */
  horizons_pred?: Partial<Record<SdsCurveRoiHorizonKey, SdsCurveRoiHorizon>>;
  now_offset?: number | null;
  sds_correlation_estimate?: {
    horizons?: Partial<Record<SdsCurveRoiHorizonKey, SdsCurveRoiHorizon & { source?: string; bin?: number; r?: number | null }>>;
    profile_used?: string;
    sds_input?: number;
  };
};

/** Minimal SDS row fields used for cluster → scenario mapping. */
...
export const SDS_REF_PROFILE_LABELS: Record<SdsRoiProfileId, string> = {
  cluster1: "μ SuperNova",
  cluster0: "μ Cl.0",
...
export const SDS_REF_PROFILE_COLORS: Record<SdsRoiProfileId, string> = {
  cluster1: MACRO_GROUP_COLORS.cluster1,
  cluster0: MACRO_GROUP_COLORS.cluster0,
  post_rialzo: MACRO_GROUP_COLORS.post_rialzo,
  post_ribasso: MACRO_GROUP_COLORS.post_ribasso,
  post_neutro: MACRO_GROUP_COLORS.post_neutro,
};
...
function interpCurve(values: (number | null)[], target: number): number | null {
  ...
  return interpolateAtOffset(pts, target, { extrapolate: true });
}
...
/** SDS clusters A–E → bonus on scenario profile scores (mirrors Python). */
...
/** Build reference value arrays on SUPERNova_OFFSETS from guide curves. */
export function guideCurvesToRefMap(
  ...
  const out: Partial<Record<SdsRoiProfileId, (number | null)[]>> = {
    cluster1: SUPERNova_HISTORY_CURVE.map((p) => p.supernovaMean),
  };
...
export function curveRoiFromSimChart(
  simRow: Record<string, unknown>,
  ...
  if (!obs) return null;
  return computeClientCurveRoiBlend(obs, simRow, refs, ctx);
}

/** Full μ blend curve (% vs T−60) on every SuperNova calendar knot. */
...
export async function loadSdsReferenceCurves(): Promise<{
  refs: Partial<Record<SdsRoiProfileId, (number | null)[]>>;
  source: string;
}> {
  const { curves, source } = await loadPredictionGuideRefCurves();
  return { refs: guideCurvesToRefMap(curves), source };
}

export function formatProfileFitShort(
  profile: SdsRoiProfileId | null | undefined,
  fit: number | null | undefined,
): string {
  if (profile == null || fit == null) return "—";
  const abbrev: Record<SdsRoiProfileId, string> = {
    cluster1: "SN",
    cluster0: "C0",
    post_rialzo: "↑",
    post_ribasso: "↓",
  ...
  return `${abbrev[profile] ?? profile} ${fit.toFixed(0)}%`;
}

export function formatHorizonRoi(value: number | null | undefined): string {
...
export function formatHorizonRoiTooltip(
  horizon: SdsCurveRoiHorizon | null | undefined,
  label: string,
): string | undefined {
  if (!horizon?.pct_vs_m60 && horizon?.pct_vs_m60 !== 0) return undefined;
  const base = `${label}: ${formatHorizonRoi(horizon.pct_vs_m60)} vs T−60`;
  if (horizon.delta_from_now == null || !Number.isFinite(horizon.delta_from_now)) return base;
  return `${base} (Δ da oggi ${formatHorizonRoi(horizon.delta_from_now)})`;
}
