...
# Apply-side robustness (mirrors cluster_cal_factor): on top of the compute-side
# variance-explained solver, require more evidence before a stored multiplier is
# applied, shrink it toward 1.0 by sample size, and skip noise-level bias.
...
def robust_regime_multiplier(regime: str) -> float:
    ...
    apply-side guards so a regime with thin/noisy evidence stays neutral even
    after the compute-side solver has stored a value.
    """
